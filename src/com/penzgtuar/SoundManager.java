...
public class SoundManager {
    ...
    public SoundManager(Context c, int max_streams, AssetManager asstMgr, boolean useOpenSl) {
        this.context = c;
    ...
    public boolean initSoundSamples() {
        ...
        for (int count = 0; count < fields.length; count++) {
            try {
                ...
                // skip bad streams
                if (filename.startsWith("daftpunk") || filename.startsWith("noise")) {
                    continue;
                }
    ...
    public int loadSample(String name, int sound) {
        ...
        if (isUseOpenSL) {
            ...
            AssetFileDescriptor afd =
                    context.getResources().openRawResourceFd(sound);
            if (afd != null) {
